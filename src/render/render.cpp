...
void render_t::render_menu_background(ImVec2 pos, ImVec2 size, float header_height, float sidebar_width)
{
    ...
    pDrawList->AddText(ImVec2(pos.x + style.WindowPadding.x, pos.y + style.WindowPadding.y),
        ImGui::GetColorU32(ImGuiCol_Text), "headless ");
    ImVec2 title_size = ImGui::CalcTextSize("headless ");
...
void render_t::render_page_content(float group_width)
{
    ...
    else if (m_iCurrentPage == ImPage_Settings)
    {
        ImGui::BeginGroup();
        {
            begin_child_styled("Application", ImVec2(group_width, ImGui::GetFontSize() * 4.5f + style.ItemSpacing.y * 2 + style.WindowPadding.y * 2));
            {
                ...
                // Make button scale with window width
...
void render_t::render_menu()
{
    ...
    ImGui::Begin("headless nightly", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBackground);
    ImGui::PopStyleVar(2);
